...
- the second *Data transfer optimisation notebook* (Exercise 2 [`lecture12_ex2.ipynb`](https://github.com/eth-vaw-glaciology/course-101-0250-00/blob/main/notebooks/lecture12_ex2.ipynb))
...
👉 Download the [`lecture12_ex3.ipynb`](https://github.com/eth-vaw-glaciology/course-101-0250-00/blob/main/notebooks/lecture12_ex3.ipynb) notebook and edit it.
"""
...
md"""
In the previous notebook ([`lecture12_ex2.ipynb`](https://github.com/eth-vaw-glaciology/course-101-0250-00/blob/main/notebooks/lecture12_ex2.ipynb)), you learned how to explicitly control part of the the on-chip memory usage, using so called "shared memory". We will learn now how to control a second kind of fast memory on-chip: registers. To this purpose we will implement the `cumsum!` function on GPU - for the sake of simplicity, we will only write it for 3-D arrays.
...
#sol=    for iz = 1:size(A,3)
#sol=        @inbounds B[ix,iy,iz] = A[ix,iy,iz]
#sol=    end
#sol=    return nothing
#sol=end
#sol=
...
#sol=    cumsum_iz = 0.0
#sol=    for iz = 1:size(A,3)
#sol=        @inbounds cumsum_iz  += A[ix,iy,iz]
#sol=        @inbounds B[ix,iy,iz] = cumsum_iz
#sol=    end
#sol=    return nothing
#sol=end
#sol=
...
#sol=    cumsum_iy = 0.0
#sol=    for iy = 1:size(A,2)
#sol=        @inbounds cumsum_iy  += A[ix,iy,iz]
#sol=        @inbounds B[ix,iy,iz] = cumsum_iy
#sol=    end
#sol=    return nothing
#sol=end
#sol=
...
#sol=    iy = (blockIdx().y-1) * blockDim().y + threadIdx().y
#sol=    iz = (blockIdx().z-1) * blockDim().z + threadIdx().z
#sol=    for ix_offset = 0 : blockDim().x : size(A,1)-1
#sol=        ix = threadIdx().x + ix_offset
#sol=        @inbounds B[ix,iy,iz] = A[ix,iy,iz]
#sol=    end
#sol=    return nothing
#sol=end
...
#sol=    iz = (blockIdx().z-1) * blockDim().z + threadIdx().z
#sol=    tx = threadIdx().x
#sol=    shmem = @cuDynamicSharedMem(eltype(A), blockDim().x)
#sol=    cumsum_ix = 0.0
#sol=    for ix_offset = 0 : blockDim().x : size(A,1)-1
#sol=        ix = threadIdx().x + ix_offset
#sol=        @inbounds shmem[tx] = A[ix,iy,iz]       # Read the x-dimension chunk into shared memory.
#sol=        sync_threads()
#sol=        if tx == 1                            # Compute the cumsum only with the first thread, accessing only shared memory
#sol=            for i = 1:blockDim().x
#sol=                @inbounds cumsum_ix += shmem[i]
#sol=                @inbounds shmem[i] = cumsum_ix
#sol=            end
#sol=        end
#sol=        sync_threads()
#sol=        @inbounds B[ix,iy,iz] = shmem[tx]       # Write the x-dimension chunk to main memory.
#sol=    end
#sol=    return nothing
#sol=end
#sol=
