...
#sol=    tx = threadIdx().x
#sol=    ty = threadIdx().y
#sol=    T_l = CuDynamicSharedArray(eltype(T), blockDim().x, blockDim().y)
...
#sol=    T_l = CuDynamicSharedArray(eltype(T), blockDim().x+2, blockDim().y+2)
#sol=    @inbounds T_l[tx,ty] = T[ix,iy]
#sol=    if (ix>1 && ix<size(T2,1) && iy>1 && iy<size(T2,2))
#sol=        @inbounds T2[ix,iy] = T_l[tx,ty] + dt*Ci[ix,iy]
#sol=    end
...
#hint=    T_l = CuDynamicSharedArray(eltype(T), blockDim().x+2, blockDim().y+2)
#hint=    @inbounds T_l[tx,ty] = T[ix,iy]
#hint=    if (ix>1 && ix<size(T2,1) && iy>1 && iy<size(T2,2))
