...
const HTML_FOLDER  = path.join("..","..","__site");
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (stat.isDirectory()) {
            ...
            for (var j = 0; j < recursed.length; j++) {
                recursed[j] = path.join(files[i], recursed[j]);
            }
...
function buildPreviews(docs) {
    var result = {};
    var prepath = path.join('..','..','__site').replace(/\\/g,'\\\\');
    var ex = new RegExp('^'+prepath,'gi');
    for (var i = 0; i < docs.length; i++) {
        var doc = docs[i];
        result[doc["id"]] = {
            "t": doc["t"],
            "l": doc["l"].replace(ex, path.sep + PATH_PREPEND)
        }
    }
    return result;
}
