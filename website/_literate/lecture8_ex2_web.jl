...
Now that you made sure the code runs as expected, launch `PorousConvection_3D_multixpu.jl` for 4000 steps on 8 GPUs at higher resolution (global grid of ) setting (keeping other parameters unchanged):
```julia
nz          = 127
nx,ny       = 2*(nz+1)-1,nz
nt          = 4000
