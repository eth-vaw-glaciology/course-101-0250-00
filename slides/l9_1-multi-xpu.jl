...
#nb # %% A slide [markdown] {"slideshow": {"slide_type": "slide"}}
md"""
![Parallel scaling](./figures/l9_par_eff.png)
"""
...
#nb # %% A slide [markdown] {"slideshow": {"slide_type": "slide"}}
md"""
![1D Global grid](./figures/l9_1D_global_grid.png)
"""
...
#md # ~~~
# <center>
#   <video width="60%" autoplay loop controls src="./figures/l9_diff_1D_2procs.mp4"/>
# </center>
#md # ~~~
...
#md # ~~~
# <center>
#   <video width="60%" autoplay loop controls src="./figures/l9_diff_1D_nprocs.mp4"/>
# </center>
#md # ~~~
...
#nb # > 💡 note: At this point, make sure to have a working Julia MPI environment. Head to [Julia MPI install](/software_install/#julia_mpi) to set-up Julia MPI. See [GPU computing on Alps](/software_install/#gpu_computing_on_alps) for detailed information on how to run MPI GPU (multi-GPU) applications on Daint.Alps.
...
#hint=        sendbuf = ??
#hint=        recvbuf = ??
#hint=        MPI.Send(??,  neighbors_x[?], 0, comm)
#hint=        MPI.Recv!(??, neighbors_x[?], 1, comm)
#hint=        A[1] = ??
#sol=        sendbuf = A[2]
#sol=        recvbuf = zeros(1)
#sol=        MPI.Send(sendbuf,  neighbors_x[1], 0, comm)
#sol=        MPI.Recv!(recvbuf, neighbors_x[1], 1, comm)
#sol=        A[1] = recvbuf[1]
    end
...
#hint=        sendbuf = ??
#hint=        recvbuf = ??
#hint=        MPI.Recv!(??, neighbors_x[?], 0, comm)
#hint=        MPI.Send(??,  neighbors_x[?], 1, comm)
#hint=        A[end] = ??
#sol=        sendbuf = A[end-1]
#sol=        recvbuf = zeros(1)
#sol=        MPI.Recv!(recvbuf, neighbors_x[2], 0, comm)
#sol=        MPI.Send(sendbuf,  neighbors_x[2], 1, comm)
#sol=        A[end] = recvbuf[1]
    end
    return
end
...
#nb # > 💡 note: Have a look at the [`l9_hello_mpi_gpu.jl`](https://github.com/eth-vaw-glaciology/course-101-0250-00/blob/main/scripts/l9_scripts/) code to get an idea on how to select a GPU based on node-local MPI infos.
