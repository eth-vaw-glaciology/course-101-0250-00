...
#nb # %% A slide [markdown] {"slideshow": {"slide_type": "fragment"}}
#nb # > 💡 note: Values reported in this notebook are for the Nvidia GH200 96GB GPU.
...
#src #########################################################################
#nb # %% A slide [markdown] {"slideshow": {"slide_type": "slide"}}
#nb # > 💡 note: Here 1 GB is 1e9 Bytes as in the publication, where the peak memory throughput of the Nvidia GH200 GPU was obtained from.
...
#src #########################################################################
#nb # %% A slide [markdown] {"slideshow": {"slide_type": "slide"}}
#nb # > 💡 note: We use `@inbounds` macro to make sure no array bounds checking is performed, which would slow down significantly.
