...
"""
if do_viz
    if (me==0) if isdir("viz3Dmpi_out")==false mkdir("viz3Dmpi_out") end end
    ...
    if (me==0) # only the master process renders
        fig, ax, plt = heatmap(xi_g, zi_g, T_v[:, ceil(Int, ny_g() / 2), :];
...
if do_viz && (it % nvis == 0)
    ...
    if me == 0
        plt[3] = T_v[:, ceil(Int, ny_g() / 2), :]
        ## display(fig)
        recordframe!(io)
        save_array(@sprintf("viz3Dmpi_out/out_T_%04d", iframe += 1), convert.(Float32, T_v))
    end
end

## after the time loop
if (do_viz && me==0) save("viz3Dmpi_out/porous_convect_3D_mpi.mp4", io) end
...
![3D porous convection MPI](./figures/l11_porous_convect_mpi_sl.png)
"""
...
#md # ~~~
# <center>
#   <video width="90%" autoplay loop controls src="./figures/l11_porous_convection_mxpu.mp4"/>
# </center>
#md # ~~~
