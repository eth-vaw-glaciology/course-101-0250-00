...
#sol A useful and more concise one may be the following one:
#sol 
#sol ```julia
#sol dAdt[2:end-1,2:end-1] .= dAdt[2:end-1,2:end-1].*(1-dmp) .+ 
#sol                          D.*( diff(diff(A[:,2:end-1],dims=1),dims=1)/dx^2 .+
#sol                               diff(diff(A[2:end-1,:],dims=2),dims=2)/dy^2 )
#sol A                     .= A .+ dt.*dAdt
#sol ```
"""
