...
#nb # > 💡 note: Divisions are precomputed as they are slower than multiplications.
...
#nb # > 💡 note: We use simple macros to enable nice syntax, in particular macros can be used also on the left side of an equal sign (learn [here](https://docs.julialang.org/en/v1/manual/metaprogramming/#man-macros) more about macros).
...
"""
#nb # > 💡 hint: Do not forget to interpolate these predefined variables into the benchmarking expression using `$` and note that you do not need to call the solver itself (`diffusion2D`)!
...
#-
## solution
#sol=t_it = @belapsed begin diffusion2D_step!($T, $Ci, $qTx, $qTy, $dTdt, $lam, $dt, $_dx, $_dy); synchronize() end
#sol=T_tot_lb = 11*1/1e9*nx*ny*sizeof(Float64)/t_it
...
"""
#nb # > 💡 hint: Make sure to use the correct function signature: `diffusion2D_step!(T2, T, Ci, lam, dt, _dx, _dy)`.
